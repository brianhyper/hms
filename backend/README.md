...
The `dev` profile seeds one account per Phase 1 role, so each role's access can be exercised without
editing the database by hand. They are created by
[20260925120000_added_phase1_dev_users.xml](src/main/resources/config/liquibase/changelog/20260925120000_added_phase1_dev_users.xml),
which is scoped to the `dev` Liquibase context and therefore can never run against a production
database (`application-prod.yml` runs only the `prod` context).

**Password for every account below: `admin`** — the same BCrypt hash as the stock `admin` row, so
there is one development password to remember. Development only.

| Login        | Authority          | Intended for                                             |
| ------------ | ------------------ | -------------------------------------------------------- |
| `reception`  | `ROLE_RECEPTION`   | Registration, appointment check-in, reading a bill        |
| `nurse`      | `ROLE_NURSE`       | Triage, vital signs                                       |
| `doctor`     | `ROLE_DOCTOR`      | Consultations, orders, prescriptions, referrals, admission |
| `lab`        | `ROLE_LAB`         | Laboratory worklist and results                           |
| `radiology`  | `ROLE_RADIOLOGY`   | Radiology worklist and results                            |
| `pharmacy`   | `ROLE_PHARMACY`    | Prescription queue, dispensing, stock movements           |
| `finance`    | `ROLE_FINANCE`     | Billing and payment collection                            |
| `superadmin` | `ROLE_SUPER_ADMIN` | Everything, including the generated CRUD escape hatches   |

Each account also carries `ROLE_USER`, which grants nothing beyond `/api/account`. The authoritative
role-to-endpoint mapping is the "PHASE 1 RBAC TABLE" block in
[SecurityConfiguration.java](src/main/java/com/hyperbrains/hms/config/SecurityConfiguration.java).
