...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Taurean.Oberbrunner-Kemmer', email: 'Darlene70@gmail.com' };
  ...
  describe.skip('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Virgie45');
      cy.get(`[data-cy="login"]`).should('have.value', 'Virgie45');

      cy.get(`[data-cy="firstName"]`).type('Flavio');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Flavio');

      cy.get(`[data-cy="lastName"]`).type('Bergstrom');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Bergstrom');

      cy.get(`[data-cy="email"]`).type('Tiffany51@hotmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Tiffany51@hotmail.com');
