...
public class ExceptionTranslator extends ResponseEntityExceptionHandler {
    ...
    private ProblemDetailWithCause getProblemDetailWithCause(Throwable ex) {
        if (
            ex instanceof com.hyperbrains.hms.service.UsernameAlreadyUsedException
        ) return (ProblemDetailWithCause) new LoginAlreadyUsedException().getBody();
        if (
            ex instanceof com.hyperbrains.hms.service.EmailAlreadyUsedException
        ) return (ProblemDetailWithCause) new EmailAlreadyUsedException().getBody();
        if (
            ex instanceof com.hyperbrains.hms.service.InvalidPasswordException
        ) return (ProblemDetailWithCause) new InvalidPasswordException().getBody();

        if (ex instanceof com.hyperbrains.hms.service.ExactPatientMatchException exactMatch) {
            // Both identifiers travel in the body so the client can offer "use the existing record"
            // as well as "create a second record anyway", without a second round trip.
            return ProblemDetailWithCauseBuilder.instance()
                .withStatus(HttpStatus.CONFLICT.value())
                .withType(ErrorConstants.EXACT_PATIENT_MATCH_TYPE)
                .withProperty("message", "error.exactPatientMatch")
                .withProperty("existingPatientId", exactMatch.getExistingPatientId())
                .withProperty("existingHospitalId", exactMatch.getExistingHospitalId())
                .build();
        }

        if (ex instanceof com.hyperbrains.hms.service.BusinessRuleViolationException businessRule) {
            // Deliberately mirrors the BadRequestAlertException contract the JHipster frontend
            // already knows how to read: message key "error.<key>" plus the entity name as params.
            // Only the status differs (409), because the payload was valid and the record was not.
            return ProblemDetailWithCauseBuilder.instance()
                .withStatus(HttpStatus.CONFLICT.value())
                .withType(ErrorConstants.DEFAULT_TYPE)
                .withProperty("message", "error." + businessRule.getErrorKey())
                .withProperty("params", businessRule.getEntityName())
                .build();
        }

        if (ex instanceof com.hyperbrains.hms.service.VitalsRejectedException vitalsRejected) {
            // 400, not 409: the record's state is fine, the submitted numbers cannot be true, so
            // this is field-level validation and the client should be able to highlight the inputs.
            //
            // A warning never reaches here. Warnings travel in a 2xx body beside the saved reading,
            // because refusing a value that a real patient could have would lose the finding.
            return ProblemDetailWithCauseBuilder.instance()
                .withStatus(HttpStatus.BAD_REQUEST.value())
                .withType(ErrorConstants.CONSTRAINT_VIOLATION_TYPE)
                .withProperty("message", ErrorConstants.ERR_VALIDATION)
                .withProperty("fieldErrors", rejectionFieldErrors(vitalsRejected.getRejections()))
                .build();
        }
    ...
    /**
     * Turns refused vital-sign readings into the same {@code fieldErrors} shape that bean-validation
     * failures produce, so a client has one way to render a rejected field whichever check refused it.
     */
    private static List<FieldErrorVM> rejectionFieldErrors(List<com.hyperbrains.hms.service.rules.VitalsValidator.Rejection> rejections) {
        return rejections
            .stream()
            .map(rejection -> new FieldErrorVM("vitalSigns", rejection.field(), rejection.message()))
            .toList();
    }
    ...
    private boolean containsPackageName(String message) {
        ...
        return Strings.CS.containsAny(message, "org.", "java.", "net.", "jakarta.", "javax.", "com.", "io.", "de.", "com.hyperbrains.hms");
    }
}
