    ...
    <div>
      ...
      <div className="table-responsive">
        {prescriptionList?.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('prescribingSource')}>
                  Prescribing Source <FontAwesomeIcon icon={getSortIconByFieldName('prescribingSource')} />
                </th>
            ...
            <tbody>
              {prescriptionList.map(prescription => (
                <tr key={`entity-${prescription.id}`} data-cy="entityTable">
                  <td>
                    <Button as={Link as any} to={`/prescription/${prescription.id}`} variant="link" size="sm">
                      {prescription.id}
                    </Button>
                  </td>
                  <td>{prescription.source}</td>
                  <td>{prescription.prescribingSource}</td>
                  <td>{prescription.status}</td>
                  <td>{prescription.visit ? <Link to={`/visit/${prescription.visit.id}`}>{prescription.visit.id}</Link> : ''}</td>
                  <td>{prescription.doctor ? prescription.doctor.login : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        onClick={() =>
                          (globalThis.location.href = `/prescription/${prescription.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        variant="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No Prescriptions found</div>
        )}
      </div>
      {totalItems ? (
