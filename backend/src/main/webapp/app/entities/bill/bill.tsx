      ...
      <div className="table-responsive">
        {billList?.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('paidAt')}>
                  Paid At <FontAwesomeIcon icon={getSortIconByFieldName('paidAt')} />
                </th>
                <th>
            ...
            <tbody>
              {billList.map(bill => (
                <tr key={`entity-${bill.id}`} data-cy="entityTable">
                  <td>
                    <Button as={Link as any} to={`/bill/${bill.id}`} variant="link" size="sm">
                      {bill.id}
                    </Button>
                  </td>
                  <td>{bill.totalAmount}</td>
                  <td>{bill.status}</td>
                  <td>{bill.paidAt ? <TextFormat type="date" value={bill.paidAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{bill.payment ? <Link to={`/payment/${bill.payment.id}`}>{bill.payment.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button as={Link as any} to={`/bill/${bill.id}`} variant="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        onClick={() =>
                          (globalThis.location.href = `/bill/${bill.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        variant="danger"
                        size="sm"
