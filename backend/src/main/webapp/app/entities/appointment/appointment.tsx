          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('scheduledDate')}>
                  Scheduled Date <FontAwesomeIcon icon={getSortIconByFieldName('scheduledDate')} />
                </th>
                <th className="hand" onClick={sort('scheduledTime')}>
                  Scheduled Time <FontAwesomeIcon icon={getSortIconByFieldName('scheduledTime')} />
                </th>
            ...
            <tbody>
              {appointmentList.map(appointment => (
                <tr key={`entity-${appointment.id}`} data-cy="entityTable">
                  <td>
                    <Button as={Link as any} to={`/appointment/${appointment.id}`} variant="link" size="sm">
                      {appointment.id}
                    </Button>
                  </td>
                  <td>
                    {appointment.scheduledDate ? (
                      <TextFormat type="date" value={appointment.scheduledDate} format={APP_LOCAL_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>{appointment.scheduledTime}</td>
                  <td>{appointment.reason}</td>
                  <td>{appointment.status}</td>
                  <td>{appointment.visit ? <Link to={`/visit/${appointment.visit.id}`}>{appointment.visit.id}</Link> : ''}</td>
                  <td>{appointment.patient ? <Link to={`/patient/${appointment.patient.id}`}>{appointment.patient.id}</Link> : ''}</td>
                  <td>
                    {appointment.department ? <Link to={`/department/${appointment.department.id}`}>{appointment.department.id}</Link> : ''}
                  </td>
                  <td>{appointment.doctor ? appointment.doctor.login : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button as={Link as any} to={`/appointment/${appointment.id}`} variant="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        onClick={() =>
                          (globalThis.location.href = `/appointment/${appointment.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        variant="danger"
                        size="sm"
